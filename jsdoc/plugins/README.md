...
- description
- kind
- name: when this one is not set at some point in time, the doclet will be ignored in the end.
...
Note that the same event instance is used as for the previous `fileBegin` event, hence 
anything you change to the event there will be visible in `fileComplete`.

#### Event: parseBegin

This is fired before the parser will parse any source files.  You might use this to
perform some setup in your plugin.

The event object will contain the following properties:

- sourcefiles: the array listing all sourcefiles

#### Event: parseComplete

This is fired when the parser is done parsing all the source files.  You might use this to
perform some postprocessing in your plugin.

The event object will contain the following properties:

- sourcefiles: the array listing all sourcefiles
- doclets: array; can be modified or re-assigned: this set of doclets will be used by
  the subsequent stages of the documentation generation process.

Note that the same event instance is used as for the previous `parseBegin` event, hence 
anything you change to the event there will be visible in `parseComplete`.
(**only in patched latest JSDoc**)
