...
%token
  ...
  PTOKEN PLEFT PRIGHT PNONASSOC PTYPE PSTART PPREC PUNION   /* reserved words: PTOKEN=%token, etc. */
...
grammar     : grammar_p1 grammar_p2 aux_procs
            ;
...
lcurl_def   : lcurl         { copy_code; } 
            ;
ptoken_def  : ptoken        { act_prec := 0; }
            ;
punion_def  : punion        { copy_union_code; } 
            ;
pleft_def   : pleft         { act_prec := new_prec_level(left); }
            ;
pright_def  : pright        { act_prec := new_prec_level(right); }
            ;
pnonassoc_def : pnonassoc   { act_prec := new_prec_level(nonassoc); }
            ;
...
epsilon_rul :               { next_section; }
            ;
lcurl_rul   : lcurl         { copy_code; } 
            ;
rcurl_rul   : rcurl         { next_section; }
            ;
...
c_id_r1     : c_id          { start_rule(ntsym($1)); }
            ;
colon_r1    : colon         { start_body; }
            ;
...
lbrace_act1 : lbrace        { copy_action; } 
            ;
...
pprec_literal : pprec literal
                            { add_rule_prec($2); }
            ;
pprec_litid : pprec litid
